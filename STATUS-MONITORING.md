...
The live status page uses the Supabase Edge Function `status-api` and never calls a public CORS proxy or fabricates uptime values. Vercel and Render Static Site both host the deployable site, while GitHub Actions provides the free scheduled trigger.
...
- `.github/workflows/status-monitor.yml` triggers a fresh probe every 5 minutes using the public GitHub repository runner (GitHub may delay scheduled jobs slightly).
