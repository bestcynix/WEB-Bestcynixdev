(function () {
  ...
  function renderInvitePreview(invite, inviteId) {
    ...
    action?.addEventListener('click', async () => { action.disabled = true; action.textContent = 'กำลังตรวจสอบ…'; try { await joinInvite(inviteId); } catch (error) { flash(error.message || 'เข้าร่วมกลุ่มไม่สำเร็จ', 'error'); action.disabled = false; action.textContent = state.user ? 'ตรวจสอบและเข้าร่วม' : 'เข้าสู่ระบบก่อนเข้าร่วม'; } });
  }
  ...
  async function joinInvite(inviteId) { if (!state.user) throw new Error('กรุณาเข้าสู่ระบบก่อนเข้าร่วมกลุ่ม'); if (!validInviteId(inviteId)) throw new Error('ลิงก์เชิญไม่ถูกต้อง'); const inviteSnap = await db.doc(`teamInvites/${inviteId}`).get(); if (!inviteSnap.exists) throw new Error('ไม่พบลิงก์เชิญ'); const invite = inviteSnap.data(); if (!invite.active || new Date(invite.expiresAt).getTime() < Date.now() || Number(invite.uses || 0) >= Number(invite.maxUses || 100)) throw new Error('ลิงก์เชิญหมดอายุหรือถูกปิดแล้ว'); const groupRef = db.doc(`teamGroups/${invite.groupId}`); const memberRef = groupRef.collection('members').doc(state.user.uid); await db.runTransaction(async (tx) => { const groupSnap = await tx.get(groupRef); const memberSnap = await tx.get(memberRef); if (!groupSnap.exists) throw new Error('ไม่พบกลุ่มงาน'); if (memberSnap.exists) return; const g = groupSnap.data(); tx.set(memberRef, { uid: state.user.uid, displayName: displayName(), email: state.user.email || '', photoURL: state.user.photoURL || null, role: 'member', online: true, lastSeen: timestamp(), joinedAt: timestamp(), inviteId }); tx.update(groupRef, { memberUids: [...(g.memberUids || []), state.user.uid], lastMemberUid: state.user.uid, lastJoinInviteId: inviteId, updatedAt: timestamp() }); tx.update(inviteSnap.ref, { uses: firebase.firestore.FieldValue.increment(1) }); }); flash('เข้าร่วมกลุ่มสำเร็จแล้ว', 'info'); await loadGroups(); await selectGroup(invite.groupId); }
